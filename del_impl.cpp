...
void Delaunay::Triangulate(std::string& triswitches){
	...
	pin->pointlist = static_cast<double *> ((void *)(&PList[0])) ;
  	...
  	pin->pointmarkerlist = (int *) nullptr;
		pin->numberofsegments = segments.size()/2;
		pin->segmentlist = static_cast<int *> ((void *)(&segments[0])) ;
		pin->segmentmarkerlist = static_cast<int *> ((void *)(&markers[0]));
		pin->numberofholes = HList.size()/2;
		pin->holelist = static_cast<double *> ((void *)(&HList[0]));
	...
	if(segments.size()>0){
		//cerr << "Using segment constraints\n";
		if(markers.size()!=segments.size()/2){
			//cerr <<"Not using segment boundary markers\n";
			pdelclass->formskeleton(tpmesh,tpbehavior, pin->segmentlist,NULL, pin->numberofsegments);
		}
		else{
			//cerr <<"Using segment boundary markers\n";
			pdelclass->formskeleton(tpmesh,tpbehavior, pin->segmentlist,pin->segmentmarkerlist, pin->numberofsegments);
		}
		pdelclass->carveholes(tpmesh,tpbehavior,pin->holelist,pin->numberofholes,pin->regionlist,pin->numberofregions);
	}
