...
/*! \file dpoint.hpp
  ...
  \bug    No known bugs.
	...
	//Changed assert to Q_ASSERT so no extra include is needed
 */
...
public:
	...
	dpoint(){ 
		Q_ASSERT( (D >= 1));//, "Dimension < 1 not allowed" );
		// move2origin(); 
	};
...
dpoint<NumType, D> CrossProd(const dpoint<NumType, D>& vector1, 
	 ...
	 Q_ASSERT(D == 3);//, "Cross product only defined for 3d vectors");
