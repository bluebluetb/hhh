/*
 * Jan 2014 - Added interface for constrained delaunay triangulations - QWB
 *
 */
...
public:
	...
	*/
	...
	struct Segment{
			int first,second;
			int marker;
			Segment(int first, int second, int marker=-1):first(first),second(second),marker(marker){}
	};
private:
	
	std::vector< Point >  PList;/*! Stores the input point list. */
	std::vector< Point >  HList;/*! Stores the input hole list. */
	std::vector< int > segments; /*! Stores the input segments for a constrained triagulation (two indices for each segment) */
	std::vector< int > markers; /*! Stores input boundary markers, one for each segment */
...
public:
    ...
    */
	...
	Delaunay(std::vector< Point >& v, std::vector< Segment > &s){
		PList.assign(v.begin(), v.end());
		for(unsigned int i=0;i<s.size();i++){
			segments.push_back(s[i].first);
			segments.push_back(s[i].second);
			if(s[i].marker>=0) markers.push_back(s[i].marker);
		}
		Triangulated = false;
	}

	Delaunay(std::vector< Point >& v, std::vector< Segment > &s, std::vector< Point > &h){
		PList.assign(v.begin(), v.end());
		HList.assign(h.begin(),h.end());
		for(unsigned int i=0;i<s.size();i++){
			segments.push_back(s[i].first);
			segments.push_back(s[i].second);
			if(s[i].marker>=0) markers.push_back(s[i].marker);
    ...
    */
	void Triangulate() { std::string s = "vzQ"; if(segments.size()>0) s="QYYzp";Triangulate( s ); }
